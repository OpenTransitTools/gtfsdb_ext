...
def to_csv(stops, feed_id, output):
    ...
    layer = feed_id + ":stops"
    source = "transit"
    n = utils.make_pelias_csv_record(layer=layer, source=source, aliases="", popularity=1.0)
    n["addendum_json_gtfs"] = ""
    ...
    for s in stops:
        ...
        n['name_json'] = utils.to_alias_json(s.stop_id, s.get_stop_code())
