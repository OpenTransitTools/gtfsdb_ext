...
from gtfsdb.util import get_csv, do_sql
...
def shared_stops_parser(stops_csv_file, db, src_feed_id="TRIMET"):
    ...
    for s in stop_dict:
        ...
        result = get_nearest_record(db, feed_id, stop_id, 0.00013, INCHES_AWAY, src_feed_id)
        if result:
            ret_val[INCHES_AWAY].append(result)
        else:
            result = get_nearest_record(db, feed_id, stop_id, 0.00033, FEET_AWAY, src_feed_id)
            if result:
                ret_val[FEET_AWAY].append(result)
            else:
                result = get_nearest_record(db, feed_id, stop_id, 0.00066, YARDS_AWAY, src_feed_id)
                if result:
